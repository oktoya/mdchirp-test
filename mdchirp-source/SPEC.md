...
- Shell mount 직후에는 아직 offline 상태인 `sync.list()`를 호출하여 로컬 draft 목록을 먼저 표시한다.
- `syncRef.current`는 연결 확인보다 먼저 등록하여, 연결 상태 콜백이 현재 Sync 인스턴스로 목록을 갱신할 수 있게 한다.
- `/api/health` 확인으로 online 전환이 감지되면 `sync.list()`를 다시 호출하여 NAS `posts-src` 목록과 로컬 draft를 `Post.id` 기준으로 결합한다.
- NAS 목록의 `hasRichSource`를 반영하여 GitHub에서 가져온 외부 글이 일반 리치 글로 잘못 표시되지 않게 한다.
- 앱 시작 과정에서는 `sync.refreshGithub()`나 `POST /api/git/refresh`를 호출하지 않는다.
...
- 글의 `media_subpath`가 GitHub 저장소 내부의 안전한 디렉터리를 가리키면, 그 디렉터리의 일반 파일도 NAS 글의 `media/` 폴더로 가져온다.
- 기존 NAS 글과 연결된 GitHub 포스트도 새로고침할 때 원격 미디어를 NAS에 보강할 수 있다.
- 미디어 폴더가 없거나 안전하지 않거나 일부 파일을 읽지 못하면 글 전체를 버리지 않고 `remote_media_missing` 경고를 기록한다.
