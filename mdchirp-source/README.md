...
- ✅ **sync 레이어** (`@mdchirp/core/sync`) — 에디터 ↔ NAS 실제 연결. `HttpAdapter`/`LocalStorageAdapter` 주입식, `NasClient`(409=결과), `Sync`(local-first + 오프라인 큐 + 충돌/연결 이벤트). NAS 목록과 로컬 draft를 `Post.id`로 결합하며, NAS의 `hasRichSource`를 목록에 반영한다. GitHub 수동 새로고침은 일반 `list()`와 분리되어 있다. 플레이그라운드 저장/발행이 실제 NAS로 감.
...
- ✅ **shell 모듈** (`@mdchirp/core/shell`) — 모듈 배치/전환 그릇. `PlatformAdapter` 주입만으로
  ...
  전역 단축키(등록/해제), 온라인·충돌 상태표시. 앱 시작 시 로컬 목록을 먼저 표시하고 NAS 연결 확인 후
  NAS 목록을 자동 갱신하며, 목록의 `↻` 버튼은 GitHub 원격 글을 NAS로 가져온 뒤 목록을 다시 읽는다.
  변환 어댑터+단축키 단위테스트, playground 에 Shell 탭으로 검증.
...
- ✅ **프론트매터 패널** (`@mdchirp/core/editor/frontmatter`) — Chirpy 전 필드 GUI(접이식 가로 영역, SplitView 소유) + 영문 slug 제안 버튼(후보 제시→사람 선택). `pin`/`math`/`mermaid`와 `toc`/`comments`/`render_with_liquid`의 서로 다른 기본값·저장 규칙을 보존하며, 체크 해제된 `false` 값도 유실하지 않는다. 순수변환 테스트와 playground 검증 완료.
...
- ✅ **GitHub 발행 실구현** — NAS가 `git add/commit/push`, PAT는 NAS에만
- ✅ **GitHub → NAS 역방향 새로고침** — 목록의 `↻` 버튼을 누르면 NAS가 원격 GitHub branch를 fetch하고 `_posts`를 스캔해 GitHub에만 있는 글을 `hasRichSource: false`인 외부 글로 가져온 뒤 목록을 다시 불러옴
- ✅ **예약 발행 API** — status=scheduled, publishAt 저장 (스케줄러는 슬롯)
- ✅ **GitHub → NAS 수동 새로고침** (`@mdchirp/backend` + core sync/shell/lister) — 목록의 `↻` 버튼을 눌렀을 때만 원격 branch를 fetch한다. 표준 날짜형 GitHub-only 글과 `media_subpath` 미디어를 NAS로 가져오고, ID·경로 충돌·잘못된 frontmatter·원격 파일 누락을 진단한다. 실패 시 기존 목록과 편집 중인 글을 유지한다.
...
## 다음 단계
1차 MVP와 이미지 첨부·기존 이미지 상세 편집, GitHub → NAS 수동 새로고침 흐름은 완료됐다.
다음 단계는 예약 시각이 되면 실제로 발행하는 스케줄러, 미사용 이미지 정리,
고급 미디어 정책, prompter/embed 등 후속 UX·확장 작업이다.
