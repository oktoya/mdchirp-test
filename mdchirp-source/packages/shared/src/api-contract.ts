...
export interface PublishResponse {
  ...
  // 발행 성공 후 갱신된 GitHub 원격 상태.
  // 발행 빌더가 mdchirp_id를 넣고 push까지 성공했으므로
  // 기존 missing_post_id 진단을 즉시 제거하기 위해 사용한다.
...
export type GitRefreshErrorCode = 'remote_refresh_failed' | 'refresh_in_progress'

/**
 * GitHub 원격 파일에서 발견한 진단 항목.
 *
 * 정상적으로 가져온 GitHub 글뿐 아니라 프론트매터 오류처럼 NAS Post로
 * 가져올 수 없는 파일도 목록 화면에서 표시할 수 있도록 별도로 반환한다.
 */
export interface RemotePostDiagnostic {
  githubPath: string
  title: string
  slug?: string
  postId?: string
