...
export function Lister(props: ListerProps): JSX.Element {
  const { posts, loading, query, onQueryChange, onOpen, onNew, onRefresh, onDelete } = props
  ...
  return (
    <div className="lister">
      <div className="lister__bar">
        ...
        {onRefresh && (
          <button type="button" className="lister__refresh" onClick={onRefresh} title="새로고침">
            ↻
          </button>
        )}
      ...
      <ul className="lister__list">
        {visible.map((p) => {
          ...
          return (
            <li
              key={p.id}
              className={'lister__item' + (external ? ' lister__item--external' : '')}
              onClick={() => onOpen(p.id)}
              role="button"
              tabIndex={0}
              onKeyDown={(e) => {
            ...
            >
              ...
              <div className="lister__meta">
                ...
                {extras.map((b) => (
                  <span key={b.key} className={'lister-badge ' + TONE_CLASS[b.tone]}>
                    {b.label}
                  </span>
                ))}
