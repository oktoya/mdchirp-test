...

{
  ...
  check(
    'GitHub import 글 → imported 배지',
    badges.some((badge) => badge.key === 'remote-imported'),
  )
}

{
  ...
  const missing = badges.find((badge) => badge.key === 'issue-remote_post_missing-0')

  check(
    '원격 글 누락 → 빨간 오류 배지',
    missing?.tone === 'red' && missing.label.includes('원격 글 없음'),
  )
}

{
  ...
  const conflictBadge = badges.find((badge) => badge.key === 'issue-slug_conflict-0')

  check('slug 충돌 → 주황 경고 배지', conflictBadge?.tone === 'orange')
}
