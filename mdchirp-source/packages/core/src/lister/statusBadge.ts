...
export type BadgeTone = 'gray' | 'yellow' | 'blue' | 'purple' | 'green' | 'orange' | 'red'
...
export function extraBadges(post: Post): Badge[] {
  ...
  if (post.remoteState === 'imported') {
    badges.push({
      key: 'remote-imported',
      label: '↓ GitHub에서 가져옴',
      tone: 'blue',
      title: '수동 새로고침에서 GitHub 글을 NAS로 가져왔습니다.',
    })
  }

  const labels: Record<NonNullable<Post['issues']>[number]['code'], string> = {
    missing_post_id: '⚠ ID 없음',
    invalid_frontmatter: '⛔ 프론트매터 오류',
    duplicate_post_id: '⚠ ID 중복',
    slug_conflict: '⚠ slug 충돌',
    remote_slug_diverged: '⚠ 원격 경로 다름',
    remote_post_missing: '⛔ 원격 글 없음',
    remote_refresh_failed: '⛔ 원격 조회 실패',
    remote_media_missing: '⚠ 커버 이미지 없음',
    incomplete_nas_post: '⛔ 발행 정보 불완전',
  }

  post.issues?.forEach((postIssue, index) => {
    badges.push({
      key: `issue-${postIssue.code}-${index}`,
      label: labels[postIssue.code],
      tone: postIssue.severity === 'error' ? 'red' : 'orange',
      title: [postIssue.message, postIssue.githubPath, postIssue.detail].filter(Boolean).join('\n'),
    })
  })
