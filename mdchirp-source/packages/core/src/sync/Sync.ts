...
import type { MediaFile, Post, PostPatch, UploadMediaOptions } from '@mdchirp/shared'
...
export class Sync {
  ...
  private async bumpDraftAfterSchedule(
    postId: string,
    publishAt: string,
  ): Promise<void> {
    const env = await this.getDraft(postId)
  ...
  async list(): Promise<Post[]> {
    if (this.online) {
      try {
        ...
        const drafts = await this.storage.all<DraftEnvelope>(
          COLLECTIONS.drafts,
        )

        // slug는 rename될 수 있으므로 NAS 목록과 로컬 draft를 Post.id로 병합한다.
        const draftsById = new Map(
          drafts.map((draft) => [draft.post.id, draft]),
        )
  ...
  async openPost(id: string, slug?: string): Promise<OpenResult> {
    ...
    const remoteSlug = await this.currentNasSlug(
      id,
   ...
   */
  private async migrateLegacyQueueItem(
    ...
    const matches = drafts.filter(
      (draft) => draft.post.slug === item.slug,
    )
  ...
  private async doFlush(): Promise<void> {
    const items = (
      await this.storage.all<StoredQueueItem>(COLLECTIONS.queue)
    ).sort((a, b) =>
