...
- 같은 ID의 로컬 draft가 `dirty=false`이면 NAS의 최신 slug·제목·rev·상태·`hasRichSource`와 원격 관측 메타데이터를 반영한다.
- 로컬 draft 본문을 유지하더라도 `hasRichSource`는 NAS 목록 값을 사용한다. 따라서 GitHub에서 가져온 `hasRichSource=false` 글이 기존 로컬 봉투 때문에 리치 원본 보유 글로 잘못 표시되지 않는다.
...
- Shell mount 직후 호출하면 현재 Sync가 offline 상태이므로 로컬 draft 목록을 먼저 반환한다.
- `/api/health` 확인 후 online 전환 콜백에서 다시 호출하면 NAS `GET /api/posts` 결과와 로컬 draft를 `Post.id`로 결합한다.
- NAS 목록과 결합할 때 clean draft에는 NAS의 최신 `hasRichSource`와 원격 관측 메타데이터를 반영한다.
- GitHub API, Git fetch 또는 `POST /api/git/refresh`를 호출하지 않는다.
- NAS가 offline이거나 NAS 목록 조회에 실패하면 로컬 draft 목록을 반환한다.
...
- ✅ GitHub 수동 새로고침: `refreshGithub()`가 `POST /api/git/refresh`를 호출하고, 동시 호출은 같은 진행 Promise를 공유한다.
- ✅ 목록 시작 흐름: 로컬 draft를 먼저 표시하고 online 전환 후 NAS 목록을 자동 결합한다.
- ✅ clean draft 목록 결합 시 NAS의 최신 `hasRichSource`와 원격 관측 메타데이터를 반영한다.
