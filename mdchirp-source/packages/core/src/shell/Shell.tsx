...
import { Sync, type ConflictInfo, type SaveResult as SyncSaveResult } from '../sync/index.js'
...
export function Shell({ platform, mode = 'full' }: ShellProps) {
  ...
  const [remoteDiagnostics, setRemoteDiagnostics] = useState<RemotePostDiagnostic[]>([])
  ...
  useEffect(() => {
    ...
    sync.onConnectivity((o) => {
      ...
      if (o) {
        // 초기 로컬 캐시 표시 후 NAS 최신 목록으로 자동 교체·병합
        void refreshList()
      }
    ...
    // checkConnectivity보다 먼저 등록해야 자동 목록 갱신이 올바른 Sync를 사용한다.
  ...
  /**
   * 사용자가 새로고침 버튼을 눌렀을 때만 실행한다.
   *
   * 1. GitHub 원격 글을 NAS와 비교·가져오기
   * 2. 성공한 경우에만 NAS 목록 다시 조회
   * 3. 실패하면 현재 posts와 이전 진단 결과를 유지
   */
  async function refreshGithub() {
    const sync = syncRef.current
    if (!sync || githubRefreshing) return
    ...
    try {
      ...
      if (!result.ok) {
        flashNotice(
          '⚠️ GitHub 새로고침 실패 — ' + (result.detail ?? '원격 저장소를 확인할 수 없습니다.'),
        )
        return
      }
      ...
      const errorCount = result.diagnostics.filter((diagnostic) =>
        diagnostic.issues.some((entry) => entry.severity === 'error'),
      ).length
      const warningCount = result.diagnostics.filter((diagnostic) =>
        diagnostic.issues.some((entry) => entry.severity === 'warning'),
      ).length
      const skippedCount = result.skippedNonstandardPaths.length

      const details = [
        result.importedPostIds.length > 0
          ? `새 글 ${result.importedPostIds.length}개 가져옴`
          : null,
        errorCount > 0 ? `오류 ${errorCount}개` : null,
        warningCount > 0 ? `경고 ${warningCount}개` : null,
        skippedCount > 0 ? `비표준 파일 ${skippedCount}개 제외` : null,
      ].filter(Boolean)

      flashNotice(
        details.length > 0
          ? `🔄 GitHub 새로고침 완료 — ${details.join(', ')}`
          : '🔄 GitHub 새로고침 완료 — 변경 없음',
      )
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      flashNotice('⚠️ GitHub 새로고침 실패 — ' + message)
    } finally {
  ...
  function handleSaveFailure(result: Extract<SyncSaveResult, { ok: false }>): string {
  ...
  async function onSave(p: Post) {
    ...
    if (res.ok) {
      ...
      flashNotice(
        res.forkedFromId ? '🌱 발행된 원본은 보존하고 새 초안으로 전환했습니다.' : '💾 저장됨',
      )
      return
    }
  ...
  async function onSchedulePublish(p: Post, publishAt: string) {
    ...
    if (res.ok) {
      flashNotice(
        res.queued ? '📥 오프라인 — 예약 요청 저장됨(연결되면 등록)' : '🗓 예약 등록 완료',
      )
    } else {
