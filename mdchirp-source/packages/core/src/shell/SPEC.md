...
- Shell mount 시 `sync.list()`만 호출한다.
- 이 초기 목록 요청은 로컬 draft와 NAS 목록만 읽는다.
- 앱 시작 시 `sync.refreshGithub()`를 호출하지 않는다.
- GitHub 연결 실패 여부가 앱 시작과 편집 화면 진입을 막지 않는다.

#### 사용자가 새로고침 버튼을 누른 경우

1. 현재 목록은 유지하고 새로고침 loading 상태를 표시한다.
2. 중복 클릭을 막는다.
3. `sync.refreshGithub()`를 호출한다.
4. 성공하면 `sync.list()`를 호출하여 GitHub-only import가 반영된 NAS 목록을 다시 읽는다.
5. `observations`를 `Post.id` 기준으로 일반 목록에 결합한다.
6. `diagnostics`는 열 수 없는 별도 진단 행으로 lister에 전달한다.
7. `skippedNonstandardPaths`가 있으면 제외한 파일 개수를 안내한다.
8. 완료 후 loading 상태를 해제한다.
