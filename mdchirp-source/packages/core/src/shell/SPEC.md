...
### 5-2. 앱 시작과 GitHub 수동 새로고침 배선 ✅

#### 앱 시작

- Shell은 Sync를 생성한 뒤 `syncRef.current`에 먼저 등록하고 `sync.checkConnectivity()`를 호출한다.
- `syncRef.current`를 연결 확인보다 먼저 등록해야 online 전환 콜백의 `refreshList()`가 현재 Sync 인스턴스를 사용할 수 있다.
- Shell mount 직후의 `refreshList()`는 로컬 draft 목록을 먼저 표시한다.
- 연결 확인 결과가 online으로 바뀌면 connectivity 콜백이 `refreshList()`를 다시 호출하여 NAS 목록과 로컬 draft를 결합한다.
- 앱 시작 시 `sync.refreshGithub()`를 호출하지 않는다.
- GitHub 연결 실패 여부가 앱 시작과 편집 화면 진입을 막지 않는다.

#### 사용자가 새로고침 버튼을 누른 경우

1. 현재 목록은 유지하고 GitHub 새로고침 loading 상태를 표시한다.
2. Shell의 `githubRefreshing`과 Sync의 진행 Promise로 중복 실행을 막는다.
3. `sync.refreshGithub()`를 호출한다.
4. backend가 동시에 진행 중인 요청을 `409 refresh_in_progress`로 거부하면 기존 목록을 유지하고 오류 안내를 표시한다.
5. 성공하면 응답의 `diagnostics`를 저장하고 `sync.list()`를 호출하여 GitHub-only import와 최신 원격 관측 메타데이터가 반영된 NAS 목록을 다시 읽는다.
6. 일반 Post의 원격 상태와 issue는 새 NAS 목록에 포함된 값을 사용한다.
7. NAS Post로 연결되지 않은 `diagnostics`는 열 수 없는 별도 진단 행으로 lister에 전달한다.
8. `skippedNonstandardPaths`가 있으면 제외한 파일 개수를 안내한다.
9. 완료 또는 실패 후 loading 상태를 해제한다.

실패한 경우:

- 현재 목록을 비우지 않는다.
- 현재 편집 중인 글을 닫지 않는다.
- 기존 NAS Post의 status를 변경하지 않는다.
- 원격 글이 없다고 오판하지 않는다.
- `remote_refresh_failed` 안내를 표시한다.

새로고침 성공 후 원격 관측 정보는 현재 실행 중인 목록 표시 데이터로 사용한다. 원격 관측만으로 편집 rev를 증가시키지 않는다.

### 5-3. ID·slug 저장 결과 배선 🚧
...
- 🚧 GitHub 수동 새로고침 배선 구현 예정:
  - 앱 시작은 `sync.list()`만 호출
  - 사용자 새로고침은 `sync.refreshGithub()` 호출
  - 성공 후 NAS 목록 재조회
  - 원격 observations와 diagnostics를 lister에 전달
  - 실패 시 기존 목록과 열린 글 유지
  - 중복 새로고침 방지
