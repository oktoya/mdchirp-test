...
export function toForm(fm: ChirpyFrontmatter): FrontmatterForm {
  ...
  return {
    ...
    // 기존 글 호환: 단수 author 와 복수 authors 를 하나의 콤마 문자열로 합친다.
    ...
    renderWithLiquid: fm.render_with_liquid ?? false,
  }
}
...
//
// 규칙:
//   - 빈 문자열/빈 배열/false 인 선택 필드는 아예 키를 넣지 않는다(깔끔한 YAML).
//   - tags 는 항상 소문자. categories 는 최대 2개(튜플).
//   - title 은 항상 포함(필수 필드).
//   - date/media_subpath 는 절대 건드리지 않는다(발행 빌더 책임, §11 경계).
// ───────────────────────────────────────────────────────────
...
export function toFrontmatter(form: FrontmatterForm): Partial<ChirpyFrontmatter> {
  ...
  if (form.toc) out.toc = true
  if (form.comments) out.comments = true
  if (form.renderWithLiquid) out.render_with_liquid = true
