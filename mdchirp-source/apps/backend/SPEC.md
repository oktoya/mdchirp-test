...
- 사용자 수동 새로고침 요청 시 git fetch → 원격 commit tree 분석 → GitHub-only 글 import 및 원격 상태 반환
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| POST | `/api/git/refresh` | 원격 GitHub branch를 fetch하고 원격 commit tree의 `_posts`를 NAS 글 목록과 비교한다. 정상 GitHub-only 글을 NAS로 import하고 원격 상태·오류 진단을 반환한다. |

#### 호출 시점

- 앱 시작 시에는 이 API를 호출하지 않는다.
- 사용자가 목록의 새로고침 버튼을 눌렀을 때만 호출한다.
- 같은 시각에 여러 새로고침 요청이 들어오면 하나의 진행 Promise를 공유하거나 후속 요청을 거부한다.
...
git fetch origin <branch>
  → fetch된 원격 branch의 commit hash 확인
  → git ls-tree로 원격 _posts 목록 조회
  → git show <commit>:<path>로 필요한 날짜형 파일 읽기
```

새로고침 중에는 다음 작업을 하지 않는다.
...
- Git push
- 원격 파일 rename/delete
- repo 작업 트리의 `_posts`를 원격 진실원으로 사용

로컬 repo의 `HEAD`, 작업 트리, 캐시된 `origin/main`, 실제 fetch 결과가 서로 다를 수 있으므로 새로고침 판정은 이번 fetch로 확인한 원격 commit을 기준으로 한다.
...
- 내용을 읽지 않는다.
- frontmatter를 파싱하지 않는다.
- NAS로 import하지 않는다.
- 일반 포스트 목록에 넣지 않는다.
- `skippedNonstandardPaths`에만 기록한다.
- 자동 rename·commit·delete하지 않는다.

날짜형 파일은 UTF-8 BOM을 제거한 뒤 다음을 검사한다.

1. 첫 줄이 `---`인지 확인
2. 닫는 `---` 경계 확인
3. YAML 파싱
4. title과 date 확인
5. `mdchirp_id`가 있으면 공백이 아닌 문자열인지 확인

날짜형 파일의 frontmatter가 잘못됐으면 NAS로 import하지 않고 `invalid_frontmatter` 진단을 반환한다.
...
- 같은 `mdchirp_id`와 같은 경로는 같은 글이다.
- 같은 `mdchirp_id`가 다른 경로에 있으면 `remote_slug_diverged`이며 자동 연결·rename하지 않는다.
- 같은 `mdchirp_id`가 여러 GitHub 파일에 있으면 `duplicate_post_id`이며 관련 파일을 자동 처리하지 않는다.
- `mdchirp_id`는 다르지만 같은 slug를 사용하면 `slug_conflict`이며 자동 import하지 않는다.
- 제목·날짜·본문 유사도는 자동 연결에 사용하지 않는다.
- NAS Post가 `status='published'`이고 `githubPath`가 원격 tree에 없으면 `remote_post_missing`으로 반환한다.
- 원격 조회에 실패하면 `remote_refresh_failed`로 반환하고 기존 글을 `missing`으로 변경하지 않는다.

#### GitHub-only 글 import

정상 날짜형이고 frontmatter 파싱에 성공한 GitHub-only 글은 새로고침 중 NAS `posts-src`로 가져온다.

예:

```text
원격: _posts/2026-06-11-blog-writing-rules.md
NAS slug: blog-writing-rules
NAS 폴더: posts-src/blog-writing-rules/
```

생성되는 Post 규칙:

- `id`: `mdchirp_id`가 있으면 해당 문자열, 없으면 `randomUUID()`
- `slug`: 파일명의 `YYYY-MM-DD-` 접두사와 `.md` 확장자를 제거한 값
- `githubPath`: 원격 `_posts` 상대경로
- `markdown`: frontmatter를 제외한 본문
- `frontmatter`: 파싱한 Chirpy frontmatter
- `tiptapJson`: `null`
- `hasRichSource`: `false`
- `status`: `published`
- `rev`: `1`
- `publishedRev`: `1`
- `publishedSlug`: import한 slug
- `createdAt`: 파싱 가능한 frontmatter date, 없으면 새로고침 시각
- `updatedAt`: 새로고침 시각
- `publishedAt`: 파싱 가능한 frontmatter date, 없으면 새로고침 시각

`Post.rev`는 NAS 저장 revision인 정수 의미를 유지한다. Git commit hash를 `Post.rev`에 넣지 않는다. 이번에 읽은 원격 commit hash는 새로고침 응답의 `remoteCommit`으로 별도 반환한다.

같은 `mdchirp_id` 또는 `githubPath`의 글이 이미 NAS에 있으면 새 NAS 폴더를 만들지 않는다. 새로고침은 동일 원격 commit과 파일에 대해 멱등이어야 한다.

#### GitHub 미디어 가져오기

GitHub 포스트의 `media_subpath`가 저장소 내부의 안전한 디렉터리를 가리키면 새로고침 시 해당 디렉터리의 일반 파일을 NAS 글의 `media/` 폴더로 가져온다.

- 외부 URL, `data:` URL, 상위 경로 탈출, `.git` 경로는 허용하지 않는다.
- 심볼릭 링크와 특수 파일은 따라가지 않는다.
- 하위 디렉터리의 일반 파일은 상대경로를 유지해 가져올 수 있다.
- 새 GitHub-only 글뿐 아니라 기존 NAS 글과 ID 또는 경로로 연결된 원격 글도 미디어를 보강할 수 있다.
- NAS에 저장한 미디어는 `Post.media[]`에 반영한다.
- 미디어 폴더가 없거나 안전하지 않거나 일부 파일 복사에 실패하면 글 전체 import를 실패시키지 않고 `remote_media_missing` warning을 기록한다.
- 미디어 가져오기는 GitHub 원격 파일을 수정하거나 삭제하지 않는다.

#### 발행 frontmatter ID

발행 빌더는 모든 발행 글의 frontmatter에 다음 필드를 출력한다.

```yaml
mdchirp_id: "<Post.id>"
```

`mdchirp_id`는 UUID 형식을 강제하지 않는다. 공백이 아닌 `Post.id` 원문을 그대로 기록하고 exact string으로 비교한다.
