...
export class PublishBuilder {
  ...
  async build(post: Post, offset: string = DEFAULT_OFFSET): Promise<BuildResult> {
    ...
    const { normalizedMarkdown, mediaFilesToCopy, missingImages } = await this.processImages(
      post.markdown,
      srcDir,
    )

    if (missingImages.length > 0) {
      throw new MissingImagesError(missingImages)
    }

    // 3. 본문 배치: repo/_posts/<fileStem>.md
    const postsDir = path.join(config.repoDir, '_posts')
    await ensureDir(postsDir)
    const postMd = await this.renderWithSubpath(
      { ...post, markdown: normalizedMarkdown },
      mediaSubpath,
      publishDate,
    )
