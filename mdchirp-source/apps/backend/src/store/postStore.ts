...
import type { Post, HistoryEntry, MediaFile, PostIssue, RemotePostState } from '@mdchirp/shared'
...
export interface RemotePostObservation {
  id: string
  githubPath?: string
...
export interface RemoteMediaImport {
  filename: string
  relativePath: string
  gitPath: string
  sourcePath: string
...
function mediaType(filename: string): MediaFile['type'] {
...
function safeMediaRelativePath(value: string): string {
  ...
  if (
    ...
    throw new Error(`안전하지 않은 미디어 상대경로입니다: ${value}`)
  }
  ...
  const rewriteString = (text: string): string =>
    text
      .replace(
        ...
        (_match, slash1: string, slash2: string) => `posts-src${slash1}${newSlug}${slash2}media`,
      )
      .replace(new RegExp(`/api/posts/${oldSlugPattern}/media`, 'g'), `/api/posts/${newSlug}/media`)
      .replace(
...
function renderPostMd(post: Post): string {
  ...
  if (fm.categories?.length) {
    const categories = fm.categories.filter(Boolean).map((value) => JSON.stringify(value))
    lines.push(`categories: [${categories.join(', ')}]`)
  }

  if (fm.tags?.length) {
    const tags = fm.tags.map((value) => JSON.stringify(value))
  ...
  if (fm.author) {
    ...
    const authors = fm.authors.map((value) => JSON.stringify(value))
  ...
  if (fm.render_with_liquid) lines.push('render_with_liquid: true')

  if (fm.media_subpath) {
    lines.push(`media_subpath: ${JSON.stringify(fm.media_subpath)}`)
  }

  if (fm.image?.path) {
    lines.push('image:')
    lines.push(`  path: ${JSON.stringify(fm.image.path)}`)

    if (fm.image.alt) {
      lines.push(`  alt: ${JSON.stringify(fm.image.alt)}`)
    }

    if (fm.image.lqip) {
      lines.push(`  lqip: ${JSON.stringify(fm.image.lqip)}`)
    }
  }
...
export class PostStore {
  ...
  private async readPost(slug: string): Promise<Post | null> {
    ...
    if (await exists(srcPath)) {
      try {
        ...
        post.tiptapJson = null
      }
    } else {
      post.tiptapJson = null
    }

    // 실제 리치 원본 파일 상태를 진실원으로 사용한다.
    post.hasRichSource = post.tiptapJson != null
  ...
  private async writePost(post: Post): Promise<void> {
    ...
    await fs.writeFile(path.join(dir, FILES.meta), JSON.stringify(metaToWrite, null, 2))
  ...
  /**
   * 성공한 수동 GitHub 새로고침의 관찰 결과를 meta.json에 반영한다.
   *
   * 본문 수정이 아니므로 rev와 updatedAt은 증가시키지 않는다.
   * 관찰 결과에 없는 글은 이전 원격 상태를 제거한다.
   * 원격 조회 자체가 실패한 경우에는 이 메서드를 호출하지 않아 이전 상태를 보존한다.
   */
  async applyRemoteObservations(observations: RemotePostObservation[]): Promise<void> {
    const operation = this.mutationTail.then(async () => {
      ...
      for (const post of posts) {
        const observation = observationById.get(post.id)
        ...
        if (observation) {
          ...
          post.issues = observation.issues?.length ? observation.issues : undefined
        } else {
  ...
  /**
   * GitHub 게시물의 media_subpath 아래에 있는 모든 일반 파일을
   * NAS 글의 media 폴더로 복사하고 Post.media에 등록한다.
   *
   * 본문 참조 여부나 파일 확장자로 가져오기 대상을 제한하지 않는다.
   * 하위 디렉터리 구조를 relativePath로 보존한다.
   * 원격 새로고침 메타 반영이므로 rev와 updatedAt은 증가시키지 않는다.
   */
  async importRemoteMedia(postId: string, assets: RemoteMediaImport[]): Promise<Post> {
    const operation = this.mutationTail.then(async () => {
      const matches = await this.findById(postId)

      if (matches.length !== 1) {
        throw new Error(`원격 미디어를 연결할 NAS 글을 찾을 수 없습니다: ${postId}`)
      }

      const post = matches[0].post
      const mediaDir = path.resolve(postDir(post.slug), FILES.mediaDir)
      ...
      for (const asset of assets) {
        const relativePath = safeMediaRelativePath(asset.relativePath)
        const filename = path.posix.basename(relativePath)
        const destination = path.resolve(mediaDir, ...relativePath.split('/'))

        if (destination !== mediaDir && !destination.startsWith(mediaDir + path.sep)) {
          throw new Error(`미디어 파일이 NAS media 폴더를 벗어납니다: ${relativePath}`)
        }

        const sourceStat = await fs.lstat(asset.sourcePath)

        if (!sourceStat.isFile() || sourceStat.isSymbolicLink()) {
          throw new Error(`일반 파일이 아닌 원격 미디어입니다: ${asset.gitPath}`)
        }
        ...
        importedMedia.push({
          id: `github-media:${post.id}:${relativePath}`,
          type: mediaType(filename),
          origin: 'local',
          filename,
          relativePath,
          nasPath: path
            .join('posts-src', post.slug, FILES.mediaDir, ...relativePath.split('/'))
            .replace(/\\/g, '/'),
          gitPath: asset.gitPath,
          sizeBytes: asset.sizeBytes,
        })
      }

      const importedPaths = new Set(
        importedMedia.map((entry) => entry.relativePath ?? entry.filename),
      )

      post.media = [
        ...(post.media ?? []).filter(
          (entry) => !importedPaths.has(entry.relativePath ?? entry.filename),
        ),
        ...importedMedia,
      ]
  ...
  async save(incoming: Post, baseRev: number): Promise<SaveResult> {
    const operation = this.mutationTail.then(() => this.saveSerialized(incoming, baseRev))
  ...
  private async saveSerialized(incoming: Post, baseRev: number): Promise<SaveResult> {
    if (
      !incoming ||
    ...
    const post: Post = {
      ...
      id: existing.id,
      rev: existing.rev + 1,
      hasRichSource: incoming.tiptapJson != null || existing.hasRichSource || false,
  ...
  private async forkPublishedPost(
    ...
    await copyIfExists(path.join(sourceDir, FILES.mediaDir), path.join(forkDir, FILES.mediaDir))
    await copyIfExists(path.join(sourceDir, FILES.source), path.join(forkDir, FILES.source))

    const rewrittenIncoming = rewriteForkPaths(incoming, originalSlug, incoming.slug)
    ...
    const fork: Post = {
      ...
      status: 'draft',
      rev: 1,
      hasRichSource: rewrittenIncoming.tiptapJson != null || existing.hasRichSource || false,
      createdAt: now,
      updatedAt: now,
    }
  ...
  async appendHistory(
    ...
    const fileName =
      `${stamp}-rev${snapshot.rev}-` + `${snapshot.lockedBy?.deviceId ?? 'unknown'}.json`
